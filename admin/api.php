...
if($method==''){
	get_code(2);
}else if($method=="get_anchor"){//    获取10个用户  全部图片   
	$sql="SELECT 
			...
			,if(f_liveimg1 !='0',f_liveimg1,'')as img1
			,if(f_liveimg2 !='0',f_liveimg2,'')as img2
			,if(f_liveimg3 !='0',f_liveimg3,'')as img3
		FROM t_anchor 
where f_examine=0 and (f_liveimg1 !='' and f_liveimg1 is not null ) and f_status=0 order by f_update_time desc limit 10";
	...
	while($rs = $rsall->fetch_assoc()){
		$count++;
		$array=array();
		if($rs['img1']!=null && $rs['img1']!="" ){
			$array[]=$rs['img1'];
		}
		if($rs['img2']!=null && $rs['img2']!="" ){
			$array[]=$rs['img2'];
		}
		if($rs['img3']!=null && $rs['img3']!="" ){
			$array[]=$rs['img3'];
		}
		unset($rs['img1']);
		unset($rs['img2']);
...
}else if($method=="set_examine"){//修改为已审核  
	$user_id=get_str_value('user_id');
	//查无此用户id	可修改
	$sql="update t_anchor set f_examine=1 where f_user_id='$user_id'";
	$rsall=mysql_query_sql(1,$sql);
	
	$sql="select id from t_anchor where f_user_id='$user_id' and f_examine=1";
	$rsall2=mysql_query_sql(1,$sql);
	
	if($rsall2->fetch_assoc()){
		get_code(0,'审核成功');
	}else{
		get_code(-1,'审核失败,id有误');
	}
}else if($method=="set_status"){//是否符合  
	$user_id=get_str_value('user_id');
	$status=get_str_value('status');
	if($status=="0" || $status=="1"){
		//查无此用户id	可修改
		$sql="update t_anchor set f_status=$status where f_user_id='$user_id'";
		$rsall=mysql_query_sql(1,$sql);
		
		$sql="select id from t_anchor where f_user_id='$user_id' and f_status=$status";
		$rsall2=mysql_query_sql(1,$sql);
		if($rsall2->fetch_assoc()){
			get_code(0,'修改成功');
		}else{
			get_code(-1,'修改失败,id有误');
		}
	}
	get_code(-1,'修改失败,状态有误');
}else{
	get_code(3);
}
...
function get_limit_sql($limit_min=1,$limit_max=100,$page_name='page',$limit_name='limit'){
	//获取页数
