...
layui.define(["element","laytpl" ,"jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        listen: function () {
             ...
             * 一级菜单模块切换
             */
			$(".layuimini-menu-left .layui-nav.layui-nav-tree .layui-nav-item").click(function(data){
				//$(".layuimini-menu-left .layui-nav.layui-nav-tree .layui-nav-itemed").removeClass('layui-nav-itemed');
				var arr=$(".layuimini-menu-left .layui-nav.layui-nav-tree .layui-nav-itemed").toArray();
				for(var i=0;i<arr.length;i++){
					if(arr[i].innerText!=data.currentTarget.innerText){
						$(arr[i]).removeClass('layui-nav-itemed');
					}
				}
			});
